<script type="text/javascript">
	...
	let edit = () => {
		...
		ApiController({
			method:"POST",
			endpoint:`agensi/${data.params.id_agensi}/edit`,
			datas: {
				nama, nama_bank, no_rekening, nama_rekening, cabang_bank
			}
		}).then(response => {
			if(response.data.message == "Update Agensi Success."){
			...
			<div class="flex flex-direction-col flex-gap-large" in:fly={{ y: -20, duration: 600 }}>
				<div class="flex flex-direction-col flex-gap-semi-large w-100">
					<div class="flex flex-direction-col flex-gap-semi-small w-100">
						<div class="title-content">Edit Agensi</div>
					...
					<div class="w-40 custom-border bg-white" style="display: flex; flex-direction: column; gap: 16px; padding: 24px; border-radius: 8px;">
						<div class="flex flex-direction-col flex-gap-semi-large">
							...
							<div class="flex flex-direction-col flex-gap-small w-100">
								...
								<input type="text" name="" class="input-nup" id="nama-agensi" value="{detailAgensi.informasi_agensi.nama == null ? '' : detailAgensi.informasi_agensi.nama}">
							</div>
							<div class="flex flex-direction-col flex-gap-small w-100">
								...
								<div class="custom-inner-card">
									<div class="flex flex-gap-regular w-100">
										<div class="flex flex-direction-col flex-gap-small w-50">
											...
											<input type="text" name="" class="input-nup" id="nama-bank" value="{detailAgensi.informasi_agensi.nama_bank == null ? '' : detailAgensi.informasi_agensi.nama_bank}">
										</div>
										<div class="flex flex-direction-col flex-gap-small w-50">
											<div class="form-input-label">Nomor Rekening</div>
											<input type="text" name="" class="input-nup" id="nomor-rekening" value="{detailAgensi.informasi_agensi.no_rekening == null ? '' : detailAgensi.informasi_agensi.no_rekening}">
										</div>
									</div>
									<div class="flex flex-gap-regular w-100">
										<div class="flex flex-direction-col flex-gap-small w-50">
											<div class="form-input-label">Nama di Rekening Bank</div>
											<input type="text" name="" class="input-nup" id="nama-pemilik" value="{detailAgensi.informasi_agensi.nama_rekening == null ? '' : detailAgensi.informasi_agensi.nama_rekening}">
										</div>
										<div class="flex flex-direction-col flex-gap-small w-50">
											<div class="form-input-label">Cabang Bank</div>
											<input type="text" name="" class="input-nup" id="cabang" value="{detailAgensi.informasi_agensi.cabang_bank == null ? '' : detailAgensi.informasi_agensi.cabang_bank}">
										</div>
									</div>
								</div>
							</div>
					...
					<div class="w-40 flex flex-gap-regular flex-end-horizontal">
						...
						<button class="btn-fill flex flex-center-vertical flex-gap-small" on:click={() => edit()}>
