<script type="text/javascript">
	...
	let getDetailDocs = () => {
		docsIdentifierTemp = window.location.href.slice(59);
		docsIdentifierReal = docsIdentifierTemp.split("/")
		console.log(docsIdentifierReal)
		ApiController({
			method: "GET",
	...
	onMount(async() => {
		...
		// getDetailSign();
		getDetailDocs();
		unit_datas = window.location.href.slice(59);
						...
						<div class="card w-100 flex flex-direction-col flex-gap-large">
							...
							{#if docs}
								<div class="flex flex-center-horizontal flex-gap-regular padding-btn-ttd">
									<a href="/super-admin/approval/detail-approval/{unit_datas_real[0]}/{unit_datas_real[1]}/{docs.fragment_surat[numberPage-1].id_fragment_surat}" class="no-decor"><button class="btn-fill-primary flex flex-center-vertical flex-center-horizontal flex-gap-small"> <span>Ke Halaman Tanda Tangan</span></button></a>
								</div>
							{/if}
						</div>
					</div>
