<script type="text/javascript">
	...
	function sendProgress() {
		...
		formData.append('status_pembangunan',form.status_pembangunan)
		formData.append('keterangan', form.keterangan)

		let postProgress = () => {
			ApiController({
				method: "POST",
				endpoint: `teknis/unit/${idUnitReal}/atur-pembangunan`,
				datas: formData
			}).then(response => {
	...
	onMount(() => {
		idUnitTemp = window.location.href.slice(35);
		idUnitReal = idUnitTemp.replace(/\D/g, "");
		console.log(idUnitReal)
	})
</script>
			...
			<div class="flex flex-direction-col flex-gap-large" in:fly={{ y: -20, duration: 600 }}>
				<div class="flex flex-direction-col flex-gap-semi-large w-100">
					<div class="flex flex-direction-col flex-gap-semi-small w-100">
						<div class="title-content">Atur Pembangunan</div>
						<div class="flex flex-gap-regular flex-center-vertical">
							...
							<div class="text-breadcrumb-active">Atur Pembangunan</div>
						</div>
					</div>
					<div class="card w-70 padding-progress">
						<div class="flex flex-direction-col flex-gap-regular">
							<div class="title-card-progress">Pembangunan</div>
