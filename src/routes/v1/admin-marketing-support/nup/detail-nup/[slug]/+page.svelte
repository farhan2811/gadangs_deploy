	...
	{#if approvalModal == 1}
		<div class="modal-backdrop">
			<div class="flex flex-center-vertical flex-center-horizontal h-100">
				<div class="card-2 flex flex-direction-col flex-gap-regular w-35" in:fly={{ y: -20, duration: 600 }} out:fly={{ y: -20, duration: 600 }}>
					<div class="title-modal">Pemberitahuan Berhasil Dikirim</div>
					<div class="desc-modal">Pesan anda telah terkirim. Semoga pesan anda dapat terpenuhi secepatnya</div>
					<button class="btn-fill-primary flex flex-center-vertical flex-gap-small flex-center-horizontal" on:click={() => {
							approvalModal = 0;
			...
			<div class="flex flex-direction-col flex-gap-large" in:fly={{ y: -20, duration: 600 }}>
				<div class="flex flex-between-horizontal flex-center-vertical w-100">
					...
					<div class="flex flex-gap-regular">
						...
						<a href="/super-admin/nup/detail-nup" class="no-decor"><button class="btn-fill-info flex flex-center-vertical flex-gap-small"><img src="/images/icons/Chat_Circle_Dots.svg"> <span>Beritahu Marketing</span></button></a>
						<button class="btn-fill-primary flex flex-center-vertical flex-gap-small" on:click={() => {
							ApiController({
								method: "POST",
								endpoint: `nup/${idNup}/request/phase-0`
							}).then(response => {
								resultApproval = response.data.message
								if (resultApproval == "Nup  requested.") {
									approvalModal = 1
								}
							})
						}}><span>Request Approval</span></button>
					</div>
				</div>
				{#if nup != null}
						...
						<div class="flex flex-gap-regular">
							...
							<div class="w-50 flex flex-gap-regular">
								...
								<div class="{nup.status_finance == "pending" ? 'label-pending' : 'label-success'} flex flex-center-horizontal flex-gap-small">
									<span>{nup.status_finance[0].toUpperCase() + nup.status_finance.slice(1, nup.status_ho.length)} Finance</span>
