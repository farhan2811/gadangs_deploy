				...
				ApiController({
					...
					ApiController({
						...
						if(response.data.privilege == "Super Admin"){
							window.location.href = '/v1/super-admin/dashboard'
						} else if(response.data.privilege == "HO Marketing"){
							window.location.href = '/v1/head-office-marketing/dashboard'
						} else if(response.data.privilege == "AMS"){
							window.location.href = '/v1/admin-marketing-support/dashboard'
						} else if(response.data.privilege == "Finance"){
							window.location.href = '/v1/finance/dashboard'
						} else if(response.data.privilege == "Teknis"){
							window.location.href = '/v1/teknis/dashboard'
						} else if(response.data.privilege == "Legal"){
							window.location.href = '/v1/legal/dashboard'
						}
