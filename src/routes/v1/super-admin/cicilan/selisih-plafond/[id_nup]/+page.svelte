<script type="text/javascript">
	...
	let atur = () => {
		...
		ApiController({
			method:"POST",
			endpoint:`cicilan/${data.params.id_nup}/atur-selisih-plafond`,
			datas:{
				nominal: nominal,
				tanggal: tanggal
			}
		}).then(response => {
			if(response.data.message == "Atur Selisih Plafond KPR Success."){
			...
			<div class="flex flex-direction-col flex-gap-large">
				<div class="flex flex-direction-col flex-gap-semi-large w-100">
					<div class="flex flex-direction-col flex-gap-semi-small w-100">
						<div class="title-content">Selisih Plafond KPR</div>
						<div class="flex flex-gap-regular flex-center-vertical">
							...
							<img src="/images/icons/Arrow_Right.svg" alt="" />
							<div class="text-breadcrumb-active">Selisih Plafond KPR</div>
						</div>
					</div>
					<div class="card w-50">
						<div class="flex flex-direction-col flex-gap-semi-large">
							<div class="flex flex-center-vertical">
								<div class="title-card-tambah-nup">Selisih Plafond KPR</div>
							</div>
							<div class="flex flex-gap-regular">
								<div class="flex flex-direction-col flex-gap-small w-50">
									<div class="title-input-nup">Nominal Dibayarkan</div>
									<input type="number" id="nominal" class="input-nup" value="{detailCicilan != null ? detailCicilan.kpr.nominal : ''}"/>
								</div>
								<div class="flex flex-direction-col flex-gap-small w-50">
									<div class="title-input-nup">Tanggal Pembayaran</div>
									<input type="date" name="" class="input-nup" id="tanggal" value="{detailCicilan != null ? detailCicilan.kpr.tanggal_pembayaran.split(" ")[0] : ''}"/>
								</div>
							</div>
						</div>
					</div>
